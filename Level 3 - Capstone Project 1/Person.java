...
/**
 * Person Class The class is used manage personnel contact information Each
 * parameter of the person class stores an array to allow for multiple accounts
 * New entries are appended to the end of each array at the last index value
 * 
 * Each account is assigned an account number and address id automatically to
 * allow search methods to locate the same account even if details on that
 * account are changed
 * 
 *
...
public class Person {
	 ...
	 * @since version 1.00
	 */
	public Person(String personType, String firstName, String lastName, String phoneNumber, String email, String street,
...

	{
		...
		this.addressID = ArrayMethods.addToIntArray(this.addressID, 1);
	}
	...
	/**
	 *
	 * Name To String Method <br>
	 * The method converts the name and surname of an account to a single string
	...
	/**
	 *
	 * Edit Personnel Account Method <br>
	 * The method requested the user to select an account type and then a specific
	 * account. The user is then provided with a list of values to edit The selected
	 * value is then altered according to the user's input If non of the selected
	 * account type exist, the user is prompted to create one Account numbers are
	 * never changed to help locate the same account even if values are changed
	 *
	 * @param statement Statement used to pass the statement on to "setNewPerson" if
	 *                  a new personnel account needs to be created
	 * @return int account number, returns the selected accounts account number / -1
	 ...
	 * @since version 3.00
	 */
	public int editPersonAccount(Statement statement) throws SQLException {
		// the user is asked to select an account type
		String personTypeToSearch = setPersonType(
				"Please selected the curret category of the account you'd like to edit:\n\n");
		...
		if (accountTypeArray[0] == -1) {
			if (InputMethods.getUserConfirmation("No " + personTypeToSearch + " Accounts currently exist.\n"
					+ "Would you like to create a new contact instead?")) {
				// creates new contact is the user selects yes
				return setNewPerson(statement, personTypeToSearch);
			...
			for (int index = 0; index < accountTypeArray.length; index++) {
				...
				accountSelectArray[index] = this.firstName[accountTypeArray[index]] + " "
						+ this.lastName[accountTypeArray[index]] + " - " + this.personType[accountTypeArray[index]];
			}
			// all available accounts are displayed for selection in a drop down box
			String menuSelection = (String) JOptionPane.showInputDialog(null, "Please Select An Account:\n\n",
					"Poised Project Management", JOptionPane.PLAIN_MESSAGE, null, accountSelectArray,
			...
			if (menuSelection == null) {
				if (InputMethods
						.getUserConfirmation("Are you sure you want to cancel entry and return to main menu?")) {
					return -1;
				}
			} else {
				...
				String[] editTypes = { "Account Type", "Name and Last Name", "Phone Number", "Email Address",
						"Physical Address" };
				String editSelection = (String) JOptionPane.showInputDialog(null,
						"Please the section of the account you'd like to edit:\n\n", "Poised Project Management",
						JOptionPane.PLAIN_MESSAGE, null, editTypes, editTypes[2]);

				if (editSelection == null) {
					return -1;

					// selection to edit Account Type
				} else if (editSelection.equals(editTypes[0])) {
					String tempPersonType = setPersonType(
							"Please select the new account type to assign to this account:\n\n");
					...
					String tempFirstName = InputMethods.getUserString(
							...
							"You need to enter their firstname in order to continue", this.firstName[selectionIndex],
					...
					String tempLastName = InputMethods.getUserString(
							...
							"You need to enter their lastname in order to continue", this.lastName[selectionIndex],
							"Are you sure you want to cancel entry?");
					...
					JOptionPane.showMessageDialog(null,
							"The first and last name has been updated for " + this.firstName[selectionIndex] + " to:\n"
									+ "Old First and Last Name: " + this.firstName[selectionIndex] + " "
									+ this.lastName[selectionIndex] + "\nNew First and Last Name: " + tempFirstName
									+ " " + tempLastName,
							"Poised Project Management", JOptionPane.INFORMATION_MESSAGE);
					...
					String tempPhoneNumber = getPhoneNumber(this.personType[selectionIndex],
							this.phoneNumber[selectionIndex]);
					...
					String tempStreet = InputMethods.getUserString(
							...
							"You need to enter their street address in order to continue", this.street[selectionIndex],
							"Are you sure you want to cancel entry?");
					...
					String tempCity = InputMethods.getUserString(
							...
							"You need to enter their current City in order to continue", this.city[selectionIndex],
							"Are you sure you want to cancel entry?");
					if (tempCity.isBlank()) {
						return -1;
					}
					String tempRegion = InputMethods.getUserString(
							...
							this.region[selectionIndex], "Are you sure you want to cancel entry?");
					...
					String tempCountry = InputMethods.getUserString(
							...
							this.country[selectionIndex], "Are you sure you want to cancel entry?");
					...
					JOptionPane.showMessageDialog(null,
							"The address has been updated for " + this.firstName[selectionIndex] + " to:\n"
									...
									+ this.postalCode[selectionIndex] + "\n\nNew Address" + "\nStreet: " + tempStreet
									+ "\nCity: " + tempCity + "\nProvince: " + tempRegion + "\nCountry: " + tempCountry
				...
				// this is done to avoid having blank information added to a profile due to the
				// user canceling midway
				// account number is never editable because it should always point towards the
				// same person's account
			}
		}
		return this.accountNum[selectionIndex];
	}

	/**
	 *
	 * Get Saved Data Method <br>
	 * The method converts saved data to array values for class method overwrites
	 * any data that might have previously been added as values
	 *
	...
	public void getSavedData(String savedData) {
		...
		// int array for account numbers is looped over to parse each value to an int
		String[] intAddArrayToCast = dataArray[11].split("\\|");
		int[] tempIntAddArray = new int[intAddArrayToCast.length];
		for (int index = 0; index < intAddArrayToCast.length; index++) {
			tempIntAddArray[index] = Integer.parseInt(intAddArrayToCast[index]);
		}
		this.addressID = tempIntAddArray;

	}

	/**
	 *
	 * getAccountIndex Method <br>
	 * Method returns the index number of the account number passed to it
	 *
	 * @param accountNumber int contains the account number of the account
	 * @return return index values of the passed account number
	 * @since version 3.00
	 */
	...
	public int getAccountIndex(int accountNumber) {
		...
		for (int index = 0; index < this.accountNum.length; index++) {
			...
			if (accountNumber == this.accountNum[index]) {
	...
	/**
	 *
	 * Get Account Type Index Method <br>
	 * Returns an array containing all the index numbers of accounts that match the
	 * same account type
	 *
	 * @param accountType       String[] array containing all account types to be
	 *                          filtered
	 * @param accountTypeNeeded String the value to be filtered for
	 * @return accountArrayIndex int[] containing an array of all the index values
	 *         of account that match the same account type
	 ...
	 *
	 * @param int accountIndex contains the index number of the project to return in
	 *            the array
	 * @return allDataToReturn String returns all data from the class and it's
	 *         parameters in a single string
	 * @since version 2.00
	 */
	public String[] getAllData(int accountIndex) {
		String[] allDataToReturn = new String[12];

		allDataToReturn[0] = Integer.toString(this.accountNum[accountIndex]);
		allDataToReturn[1] = this.personType[accountIndex];
		allDataToReturn[2] = this.firstName[accountIndex];
		allDataToReturn[3] = this.lastName[accountIndex];
		allDataToReturn[4] = this.phoneNumber[accountIndex];
		allDataToReturn[5] = this.email[accountIndex];
		allDataToReturn[6] = this.street[accountIndex];
		allDataToReturn[7] = this.city[accountIndex];
		allDataToReturn[8] = this.region[accountIndex];
		allDataToReturn[9] = this.country[accountIndex];
		allDataToReturn[10] = this.postalCode[accountIndex];
		allDataToReturn[11] = Integer.toString(this.addressID[accountIndex]);
	...
	/**
	 *
	 * Get Last Name Method <br>
	 * Returns all account data as a single string for saving in a txt file
	 *
	 * @param accNum int the account number of the last name to be retrieved
	 * @return lastNameToReturn String the Last Name value of the account associated
	...
	/**
	 *
	 * Get Phone Number Method <br>
	 * Requests the user enter a valid South African phone number Phone numbers must
	 * start with + and contain the dialing code For example: +27 78 258 2424 Method
	 * automatically removes blank spaces and does not count them towards the total
	 * length
	 *
	 * @param tempPersonType String contains the account type to be displayed when
	 *                       requesting a phone number
	 * @param currentNum     String contains the pre-filled value for the number
	 *                       field
	 * @return userSelection String containing phone number entered / or "" if the
	 *         user canceled entry
	 * @since version 2.00
	 */
	private String getPhoneNumber(String tempPersonType, String currentNum) {
		...
		String valueToRequest = "Please enter a South African phone Number for the " + tempPersonType
		...
		while (!validInput) {
			...
			userSelection = (String) JOptionPane.showInputDialog(null,
					(displayError ? errorToDisplay : "") + valueToRequest, "Poised Project Management",
					(displayError ? JOptionPane.WARNING_MESSAGE : JOptionPane.DEFAULT_OPTION), null, null,
			...
			if (userSelection == null) {
				...
				if (areaCodeSymbol.contains(Character.toString(userSelection.charAt(0)))
						&& userSelection.length() == 12) {
	...
	/**
	 *
	 * Get Valid Email Method <br>
	 * Requests the user enter a valid email address The method confirms the entry
	 * contains an "@" and is at least 6 chars long
	 *
	 * @param tempPersonType String contains the account type to be displayed when
	 *                       requesting an email
	 * @param currentEmail   String contains the pre-filled value for the email
	 * @return emailToReturn String containing phone number entered / or "" if the
	 *         user canceled entry
	 * @since version 2.00
	 */
	...
	/**
	 *
	 * Get Account Number Method <br>
	 * Requests the user select an account type and then displays a list of accounts
	 * associated with that account type If no matching account types are found the
	 * user is requested to create a new account The account number is returned
	 * after an account is selected or created
	 *
	 * @param statement    Statement used to pass the Statement on to the
	 *                     "setNewPerson" method
	 * @param accType      String contains the account type filter and display
	 *                     related account of
	 * @param isNewAccount boolean overwrites request to select an account and skips
	 *                     to account creations process if set to true
	 * @throws SQLException if an error is encountered with SQL queries
	 * @return accNumber int containing account number of the selected account / or
	 *         -1 if the user canceled entry
	 * @since version 3.00
	 */
	public int getAccountNumber(Statement statement, String accType, boolean isNewAccount) throws SQLException {
		int accNumber = 0;
		...
		if (accountTypeArray[0] == -1 && !isNewAccount) {
			...
			if (createNewAcc) {
				int newAccount = setNewPerson(statement, personTypeToSearch);
				return newAccount;
			...
			String menuSelection = (String) JOptionPane.showInputDialog(null,
					"Please Select the " + personTypeToSearch + "\n\n", "Poised Project Management",
					JOptionPane.PLAIN_MESSAGE, null, accountSelectArray, accountSelectArray[0]);
			// if the user cancels the method returns -1 to signal that the process was
			// canceled
			if (menuSelection == null) {
				accNumber = -1;
			...
			accNumber = setNewPerson(statement, personTypeToSearch);
			return accNumber;
		}
		return accNumber;
	}
	...
	public static String setPersonType(String displayRequest) {
		...
		String personSelection = (String) JOptionPane.showInputDialog(null, displayRequest, "Poised Project Management",
				JOptionPane.PLAIN_MESSAGE, null, personOptions, "Architect");
	 ...
	 *
	 * Set Personnel Type Method <br>
	 * Displays a selection to create or edit existing accounts The option to edit
	 * is removed is it's detected that no accounts currently exist
	 *
	 * @return personSelection String containing the selection to edit or create a
	 *         new account
	 * @since version 1.00
	 */
	public String setCreateOrEdit() {
		...
		String personSelection = (String) JOptionPane.showInputDialog(null, "Please Select Your Task:\n\n",
				"Poised Project Management", JOptionPane.PLAIN_MESSAGE, null, personOptions, "Create New Project");
	...
	/**
	 *
	 * Set Account Num Method <br>
	 * Appends Account Number Array with the next account number in sequence This
	 * method is used when new account are created or added to assign a new account
	 * number
	 * 
	 * @see setNewPerson
	 * @since version 1.00
	...
	/**
	 *
	 * Set New Account Method <br>
	 * Used to create a new account
	 * 
	 * @param statement SateStatement used to pass on the value you to the
	 *                  "getAccountNumber" method
	 * @param accType   String contains account type to assign to the new account
	 * @throws SQLException if an error is encountered with SQL queries
	 * @return getAccountNumber Int - sets new account
	 * @see getAccountNumber
	 * @since version 3.00
	 */
	public int setNewAccount(Statement statement, String accType) throws SQLException {
		boolean isNewAccount = true;
		return getAccountNumber(statement, accType, isNewAccount);

	}

	/**
	 *
	 * Set New Personnel Method <br>
	 * Used to create a new account and append it to the current class arrays It
	 * detects if an account currently exists or does not If no account is detected,
	 * the index position 0 is overwritten Alternatively the new account is appended
	 * to the existing arrays
	 * 
	 * @param statement   Statement used contact SQL server to create new unused
	 *                    Address ID Key
	 * @param accountType String contains account type to assign to the new account
	 * @throws SQLException if an error is encountered with SQL queries
	 * @return accountNum int contains account number of new account / returns -1 if
	 *         the process was canceled
	 * @see editPersonAccount
	 * @since version 3.00
	 */
	public int setNewPerson(Statement statement, String accountType) throws SQLException {
		...
		String tempFirstName = InputMethods.getUserString("Please enter the " + tempPersonType + "'s firstname",
				"You need to enter their firstname in order to continue");
		if (tempFirstName.isBlank()) {
			return -1;
		}
		String tempLastName = InputMethods.getUserString("Please enter the " + tempPersonType + "'s lastname",
		...
		String tempStreet = InputMethods.getUserString("Please enter the " + tempPersonType + "'s street address",
				"You need to enter their street address in order to continue");
		if (tempStreet.isBlank()) {
			return -1;
		}
		String tempCity = InputMethods.getUserString("Please enter the " + tempPersonType + "'s current City",
				"You need to enter their current City in order to continue");
		if (tempCity.isBlank()) {
			return -1;
		}
		String tempRegion = InputMethods.getUserString("Please enter the " + tempPersonType + "'s current Province",
				"You need to enter their current Province in order to continue");
		if (tempRegion.isBlank()) {
			return -1;
		}
		String tempCountry = InputMethods.getUserString("Please enter the " + tempPersonType + "'s current Country",
				"You need to enter their current Country in order to continue", "South Africa",
				"Are you sure you want to cancel entry?");
		if (tempCountry.isBlank()) {
			return -1;
		}
		...
		if (this.personType[0].equals("None") || this.personType[0].isBlank()) {
			...
			// adds new account number to account number list and adds address ID
			setAccountNum();
