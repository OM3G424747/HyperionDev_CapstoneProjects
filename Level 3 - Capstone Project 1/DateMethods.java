...
/**
 * Date Method Class The class contains methods use to set a valid date or
 * confirm if a date is past a specified date
 *
...
public class DateMethods {
	...
	/**
	 *
	 * Get Date Array Method <br>
	 * The method returns a new array containing the numeric values of the date
	 * passed
	 *
	 * @param passedDate String containing containing a date formatted in DD-MM-YYYY
	 * @return Int Array containing the number values of the Day[0], Month[1] and
	 *         Year[2]
	 * @see getDateCheck
	 * @since version 1.00
	 */
	...
	/**
	 *
	 * Get Date Check Method <br>
	 * Checks if the date passed is matching or older than today's date The method
	 * returns true if the date is matches or is older than today's date
	 *
	 * @param dateToCheck String containing containing a date formatted in
	 *                    DD-MM-YYYY
	 ...
	 *
	 * Get Date Check Method <br>
	 * Checks if the date passed is matching or older than the date passed The
	 * method returns true if the date is matches or is older than the passed date
	 *
	 * @param dateToCheck String containing containing a date formatted in
	 *                    DD-MM-YYYY
	 * @param dueDate     String containing containing a date formatted in
	 *                    DD-MM-YYYY
	 ...
	 *
	 * Set Year Method <br>
	 * Requests the user to select a valid year within the defined year range The
	 * range of years available is determined from the current year onward A year
	 * range of 0 will result in only 1 year being available
	 *
	 * @param yearRange Int containing current year range to display
	 * @return String containing the selected year / "" is returned if the user
	 *         cancels
	 * @see setDate
	 * @see setMonth
	 * @since version 1.00
	...
	private static String setYear(int yearRange) {
		...
		// sets bool to enable while loop until the user selects a valid option or
		// cancels
		...
		while (!isValidSelection) {
			...
			selectYear = (String) JOptionPane.showInputDialog(null, "Please select the due year for the project:\n\n",
					"Poised Project Management", JOptionPane.PLAIN_MESSAGE, null, yearSelection, currentYear);
	 ...
	 *
	 * Set Month Method <br>
	 * Requests the user to select a valid month If the passed year matches the
	 * current year, only the remaining months in the year will be displayed as
	 * available options
	 *
	 * @param selectedYear String containing user selected year
	 * @return String containing the selected month / "" is returned if the user
	 *         cancels
	 * @see setDate
	 * @see setYear
	 * @since version 1.00
	...
	private static String setMonth(String selectedYear) {
		...
		// confirm year selection and if it's the current year, then the array should
		// only show the current month till the end of the year
		// sets starting month to be altered based on the year selection to avoid
		// selecting a month in the past
		int startingMonth = 1;
		String[] availableMonths = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov",
				"Dec" };

		// if the current year was selected as a due year, the available months are
		// reduced to avoid duplicates
		...
		while (!isValidSelection) {
			...
			selectMonth = (String) JOptionPane.showInputDialog(null, "Please select the due month for the project:\n\n",
					"Poised Project Management", JOptionPane.PLAIN_MESSAGE, null, monthSelection,
	 ...
	 *
	 * Set Day Method <br>
	 * Requests the user to select a valid day/date from within a month If the
	 * passed year and month matches the current year and month, only the remaining
	 * days in the month will be displayed as available options
	 *
	 * @param selectedMonth String containing user selected month
	 * @param selectedYear  String containing user selected year
	 * @return String containing the selected day / "" is returned if the user
	 *         cancels
	 * @see setDate
	 * @see setMonth
	 * @see setYear
	...
	private static String setDay(String selectedMonth, String selectedYear) {
		...
		if (yearSelection % 4 == 0) {
			...
			// confirms if the selected month is in the 2nd half of the year to determine
			// available days
			if (monthSelection <= 7) {
		...
		// if it does the starting date is set to limit the available days for the
		// selection
		...
		while (!isValidSelection) {
			selectDay = (String) JOptionPane.showInputDialog(null, "Please select the due day for the project:\n\n",
					"Poised Project Management", JOptionPane.PLAIN_MESSAGE, null, daySelection,
	 ...
	 *
	 * Set Date Method <br>
	 * Requests the user to select a valid Year, Month and Day The selection is then
	 * returned in a String formatted as DD-MM-YYYY
	 *
	 * @param yearRange int containing the available year range
	 * @return String containing the selected date / "" is returned if the user
	 *         cancels
	 * @see setMonth
	 * @see setYear
	 * @see setDay
	 ...
	 * @since version 1.00
	 */
	// method used to set a date beyond today and return it in the format
	// "dd-mm-yyyy"
	public static String setDate(int yearRange) {
		...
		while (isEnteringDate) {
			...
			if (selectedDay.equals("")) {
				...
				// if all values were selected correctly they are added together in a valid
				// format to be returned
				setDate = selectedDay + "-" + selectedMonth + "-" + selectedYear;

				// if the current date matches today, the user is warned the project will be
				// marked as overdue
